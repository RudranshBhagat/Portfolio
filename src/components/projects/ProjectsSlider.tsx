  ...
  const createSlide = useCallback(
    (side: "left" | "right", idx: number): HTMLDivElement => {
      ...
      const mediaHtml = isVideo
        ? `<video src="${videoSrc}" loop muted playsinline preload="none" class="w-full h-full object-cover block will-change-transform"></video>`
        : `<img src="${mediaSrc}" alt="${p.title}" class="w-full h-full object-cover block will-change-transform" />`;
